"""
Event publisher service for publishing events to Kafka.
Implements reliable event publishing with retry logic and error handling.
"""
...
import time
import uuid
from src.models.events import BaseEvent, ContactFormSubmittedEvent, EventType
...
class EventPublisher:
    ...
    def __init__(self, bootstrap_servers: Optional[str] = None):
        ...
        self.logger.info(f"Initializing EventPublisher with bootstrap_servers={self.bootstrap_servers}")
        ...
        self.logger.info(f"Producer config: {self.producer_config}")
    ...
    def publish_contact_form_submitted(self, event_data: Dict[str, Any], 
                                     correlation_id: Optional[str] = None) -> Optional[BaseEvent]:
        """
        Convenience method for publishing ContactFormSubmitted events.
        
        Args:
            event_data: The contact form data
            correlation_id: Optional correlation ID for tracing
            
        Returns:
            BaseEvent: The published event if successful, None otherwise
        """
        try:
            event = ContactFormSubmittedEvent(
            ...
            if self.publish_event(event, "contact_form"):
                return event
            else:
                return None
                
        except Exception as e:
            self.logger.error(f"Error creating ContactFormSubmitted event: {e}")
            return None
...
def close_event_publisher():
    """Close the global event publisher instance."""
    global _event_publisher
    if _event_publisher:
